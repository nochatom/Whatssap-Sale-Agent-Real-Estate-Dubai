...
**How this changes the pipeline now:** at the time this happened, there was no `offer-config.md` — the deflection was the only honest option. That is no longer true. Pricing now exists and is flat, not scope-dependent (§ Pricing in `offer-config.md`): $149 per property regardless of platform or exact length. **A price-first asker who asks twice gets the number on the second ask.** Formatting/platform questions are still worth asking, but never in place of the price when the client has asked for it twice — ask the platform question *and* give the price in the same reply, or give the price alone. Never make price conditional on information that doesn't actually change it.
